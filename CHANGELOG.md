...
- **`references/DELTAS.template.md`:** a fork-customization template. Forkers copy it to `DELTAS.md` at their fork root to record local deviations in one place that upstream never touches, so `git merge upstream/main` stays conflict-free. Pointer added to the README fork section. (FORK_INSIGHTS.md #50; pattern from the TrevAtArena fork)
